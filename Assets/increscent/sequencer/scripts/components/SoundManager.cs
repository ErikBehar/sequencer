...
public class SoundManager : MonoBehaviour
{
    ...
    {
        if (currentMusicAudioSource.clip != null && currentMusicAudioSource.volume > .02f && currentMusicAudioSource.isPlaying && !musicFadeOutTweener.IsTweening(musicFadeOutTweener.target))
    ...
    public void stopAllPlayingSounds()
    {
        ...
        playSfx(nullSoundName, 1f, SoundInterruptType.DontInterruptButInterruptOthers);
    }
        ...
        if (!muteAllSound && !muteSfx)
        {
            switch (interruptType)
            {
                ...
                case SoundInterruptType.DontInterruptButInterruptOthers:
                    ...
                    playClipOnlyIfSimilarNotPlaying(soundClip, dontInterrupt, volume);
                    break;
                case SoundInterruptType.StopAllPlayMe:
                    stopAllSfx();
                    play(soundClip, volume);
                    break;
            }
