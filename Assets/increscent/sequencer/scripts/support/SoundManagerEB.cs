...
public class SoundManagerEB : MonoBehaviour
{
    static public SoundManagerEB _instance = null;
        #if !REMOVE_AUDIO
    ...
    static public SoundManagerEB Get()
    {
        return _instance;
    }
    ...
    void Awake()
    {
        ...
        if (_instance != null)
        {
            Debug.Log("Found a previous SoundManagerEB instance! Destroying this one!");
            Destroy(gameObject);
            return;
...
#region Play Music  
    ...
    {
        if (currentMusicAudioSource.clip != null && currentMusicAudioSource.volume > .02f && currentMusicAudioSource.isPlaying && !musicFadeOutTweener.IsPlaying())
    ...
    void actuallyPlayMusic(AudioClip musicClip, float volume)
    {
        if (musicClip == null)
        {
            debugMusic("Tried to play music but SoundManagerEB could not find it!");
            return;
        }   
        ...
        if (currentMusicAudioSource.isPlaying)
        {
            ...
            if (musicFadeOutTweener.IsPlaying() || musicFadeInTweener.IsPlaying())
            {
    ...
    public void stopMusic()
    {
        ...
        musicAudioSourceA.Stop();
        musicAudioSourceB.Stop();
    }
...
#region Play Sound Effects
    ...
    public void playRandomSfxFromSet(string setName, float volume)
    {
        ...
        {
            string soundName = sfxSets [setName] [UnityEngine.Random.Range(0, sfxSets [setName].Length)];
            playSfx(soundName);
        }
    }
    ...
    //same as above for compatability with some older versions
